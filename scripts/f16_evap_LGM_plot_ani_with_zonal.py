# -*- coding: utf-8 -*-
"""
Created on Fri Nov  1 16:44:05 2019

@author: Shihan Li
plot the evaporation during LGM
"""
...
qflx = data_qflx.variables['QFLX'][:]* unit_conversion_constant/1000
...
ax1.coastlines()
#draw controuf map
upper_limitation = np.max(qflx_last_1kyr)
...
t = -time_last_1kyr[0]
ax2.plot(lat, qflx_zonal[0,:], linewidth=3.0, color = 'b', label='%.2f ky BP' %t)
...
def animate(i): 
    ax1.clear()
    ax2.clear()
    contf1 = ax1.contourf(lon,lat,qflx_last_1kyr[i*10,:,:] - qflx_1850_1980_ave,
                          levels = np.linspace(-100,50,51), extend = 'both', 
                          projection=ccrs.PlateCarree())
    ax1.coastlines()
    ...
    ax1.xaxis.set_major_formatter(LongitudeFormatter())
    ax1.yaxis.set_major_formatter(LatitudeFormatter())
    ax2.plot(lat, qflx_zonal[i*10,:],linewidth=3.0, color = 'b', label='%.2f ka BP' %t)
    ax2.plot(lat, qflx_1850_1980_zonal, linewidth = 2.0, color = 'r', linestyle = '--', 
             label = '1850-1980 AD')
    ax2.set_ylim([0,175])
    ax2.legend(fontsize = 20)
    ax2.set_xlabel('°N',fontsize = 16)
    ...
    ax2.tick_params(labelsize=16) 
    ax2.set_ylabel("cm/year",fontsize = 16)
    ax1.set_title('Evaporation difference (%.2f ka BP)' %t, fontweight = 'bold', fontsize = 20  )
    ax2.set_title('Zonal average evaporation rate (%.2f ka BP)' %t,fontweight = 'bold', fontsize = 20 )
    #plt.suptitle('%.2f ka BP' %t,fontsize = 30, y=0.92)
     
    
anim = animation.FuncAnimation(fig, animate, frames=len(time_last_1kyr)//10, interval=800,blit=False)
anim.save('qflx_difference_LGM_modern_with_zonal.gif', writer='imagemagick')
